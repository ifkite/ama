...
from mongoengine import DynamicDocument, IntField, StringField, ListField, EmbeddedDocument, DateTimeField, DynamicField
...
class Question(DynamicDocument):
    ...
    content_type = StringField(choices=Q_TYPE, default=HIDE)
    ...
    def __repr__(self):
        return unicode('type{0} id<{1}>'.format(str(type(self)), str(self.id)))
